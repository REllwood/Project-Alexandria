...
2. Create `Sources/<slug>.md` (from the `_templates/source.md` shape): frontmatter with `type: source`, `client`, `project`, `kind`, `source_file`, `source_hash`; body = tight summary + key points.
...
5. **Decisions**: any decision/commitment → `Decisions/<slug>.md` (ADR shape), linking source + deciders.
